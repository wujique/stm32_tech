...
>  这两个函数的功能，注释说的很清楚。
>
> 第1个函数是读整个IO的状态，所以返回值是一个uint16_t，也就是一个16位的数，正好对应一组IO口。
>
...
> 按下对应按键时将对应数码管显示为0或1，表示按键时按下还是松开。
...
**重复读，当连续多次都是低点平时，按键就是低电平了。**

这时很多例程去抖动的方法。方法是对的，但是状态抽象不对，状态抽象不对，程序写的逻辑就不好。
...
所以我们要把按键驱动整理位一个模块。按键驱动扫描到按键后，放在缓冲。
